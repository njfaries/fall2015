...
void sobelize(char* input_filename, char* output_filename)
{
  ...
  for (i = 1; i < height - 1; i++) {
    ...
    for (j = 1; j < width - 1; j++) {
      ...
      value = abs((image[4*width*(i-1) + 4*(j-1)] + 2 * image[4*width*(i-1) + 4*j] + image[4*width*(i-1) + 4*(j+1)]) - (image[4*width*(i+1) + 4*(j-1)] + 2 * image[4*width*(i+1) + 4*j] + image[4*width*(i+1) + 4*(j+1)])) + abs((image[4*width*(i-1) + 4*(j+1)] + 2 * image[4*width*i + 4*(j+1)] + image[4*width*(i+1) + 4*(j+1)]) - (image[4*width*(i-1) + 4*(j-1)] + 2 * image[4*width*i + 4*(j-1)] + image[4*width*(i+1) + 4*(j-1)]));
